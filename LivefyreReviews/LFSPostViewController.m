...
- (IBAction)postClicked:(UIBarButtonItem *)sender
{
    ...
    if (userToken != nil) {
        ...
        if (title.length == 0 ) {
            UIAlertView * alertView=[[UIAlertView alloc]initWithTitle:@"Opps" message:@"Your review must include a title" delegate:self cancelButtonTitle:@"OK" otherButtonTitles: nil];
            [alertView show];
        }
        else if (text.length == 0 ) {
            UIAlertView * alertView=[[UIAlertView alloc]initWithTitle:@"Opps" message:@"Your review must include a Body" delegate:self cancelButtonTitle:@"OK" otherButtonTitles: nil];
            [alertView show];
        }
        else if (((int) value<1  && (int) value > 100) || self.writeCommentView.ratingPost==nil){
            UIAlertView * alertView=[[UIAlertView alloc]initWithTitle:@"Opps" message:@"Your review must include a star rating" delegate:self cancelButtonTitle:@"OK" otherButtonTitles: nil];
            [alertView show];
