...
canBeDismissedByUser:(BOOL)dismissingEnabled
{
    ...
    if ((self = [self init]))
    {
        ...
        if ([buttonTitle length])
        {
            ...
            [self.button setBackgroundColor:[UIColor colorWithRed:16/255.0 green:151/255.0 blue:236/255.0 alpha:0.95]];
            ...
            self.button.titleLabel.font = [UIFont boldSystemFontOfSize:13.0];
            ...
            [self.button setTitleEdgeInsets:UIEdgeInsetsMake(0.f, 0.f, 0.f, -60.f)];
            self.button.contentEdgeInsets = UIEdgeInsetsMake(0.0, 5.0, 0.0, 60.0);
        ...
        UIButton *newReviewsButton=[[UIButton alloc]init ];
        newReviewsButton.frame= CGRectMake(5, 5, 25, 20);
        
        [newReviewsButton setBackgroundColor:[UIColor colorWithRed:0/255.0 green:0/255.0 blue:0/255.0 alpha:0.15]];
        newReviewsButton.titleLabel.font = [UIFont boldSystemFontOfSize:13.0];
        [newReviewsButton setTitle:title forState:UIControlStateNormal];
        //        [newReviewsButton sizeToFit];
        //corner radious for button
        
        UIImageView *updateArrow=[[UIImageView alloc]initWithFrame:CGRectMake(122, 13, 10, 5)];
        updateArrow.image=[UIImage imageNamed:@"icon_arrow_up_white.png"];
        [updateArrow sizeToFit];
        [self.button addSubview:updateArrow];
        CALayer *btnLayer = [newReviewsButton layer];
        [btnLayer setMasksToBounds:YES];
        [btnLayer setCornerRadius:8.0f];
        
        CALayer *btnLayer1 = [self.button layer];
        [btnLayer1 setMasksToBounds:YES];
        [btnLayer1 setCornerRadius:15.0f];
        
        [self.button addSubview:newReviewsButton];
...
- (CGFloat)updateHeightOfMessageView
{
    ...
    if (self.button)
    {
        self.button.frame = CGRectMake(self.frame.size.width/2-self.button.frame.size.width/2,// - self.textSpaceRight,
                                       10,//round((self.frame.size.height / 2.0) - self.button.frame.size.height / 2.0),
                                       self.button.frame.size.width,
                                       30);
    }
